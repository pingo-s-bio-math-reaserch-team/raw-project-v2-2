...
def encode_glioma_table(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    The encoded table keeps survival labels separate from the clinical phenotype
    node. This prevents leakage when SRIS is later tested as a survival predictor.
    """
    ...
    # Direct clinical/outcome labels retained for downstream validation, not
    # included in the default C node except grade/age/KPS/purity phenotype.
...
def build_fixed_phase1_sheaf() -> SheafSpec:
    ...
    nodes = {
        ...
        "C": NodeDefinition(
            name="C",
            description="clinical phenotype state without survival leakage",
            features=("grade_risk_z", "age_z", "kps_low_z", "purity_low_z"),
        ),
    }
    ...
    edges = [
        ...
        EdgeDefinition(
            ...
            rationale="Genomic risk should align with grade/KPS/age/purity phenotype, excluding survival outcome.",
        ),
        EdgeDefinition(
            name="R_to_C",
            source="R",
            ...
            rationale="Regulatory state should align with clinical phenotype severity.",
        ),
    ]
